...
class TestCalendarRepository(unittest.TestCase):
    ...
    def test_find_by_username(self):
        ...
        event_repository.create(Event(content="testing a", date="2025-12-09", user=vici))
        event_repository.create(Event(content="testing b", date="2025-12-08", user=testi))
        
        vici_events = event_repository.find_by_username(self.user_vici.username)
        self.assertEqual(len(vici_events), 1)
        self.assertEqual(vici_events[0].content, "testing a")
        
        testi_events = event_repository.find_by_username(self.user_testi.username)
